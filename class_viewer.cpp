...
// Impressão
void print_constant_entry(u2 index, const std::vector<ConstantPoolEntry> &cp) {
  const ConstantTag tag = cp[index].first;
  const ConstantInfo &info = cp[index].second;
...
void print_read_fields(u2 index, const FieldInfo entry, const ClassFile &cf) {
  ...
  std::cout << "Attribute bytes count = " << entry.attributes_count << " "
            << std::endl;
  print_read_attributes(entry.attributes_count, entry.attributes, cf);
}
    ...
    switch (opcode) {
            ...
            for (u4 j = 0; j < num_offsets; j++) {
                u4 offset = (static_cast<u4>(code.code[read_idx]) << 24) | 
                            ...
                            (static_cast<u4>(code.code[read_idx + 2]) << 8) | 
                             static_cast<u4>(code.code[read_idx + 3]);
                read_idx += 4;
                std::cout << "\t\t      " << static_cast<int32_t>(low + j) << ": " << static_cast<int32_t>(offset) << std::endl;
            }
        ...
        case 171: { // lookupswitch
            ...
            for (u4 j = 0; j < npairs; j++) {
                ...
                std::cout << "\t\t      " << static_cast<int32_t>(match) << ": " << static_cast<int32_t>(offset) << std::endl;
            }
            u4 total_bytes_read = read_idx - i;
            i += total_bytes_read;
            break;
        ...
        case 196: { // wide
            ...
            if (opcode_wide == 132) { // iinc
                ...
                std::cout << "iinc (var #" << index_local << " by " << static_cast<int16_t>(constante) << ")";
                i += 6; // opcode + wide_opcode + 2*u2
            } else { 
                // iload, lload, fload, dload, aload, 
                // istore, lstore, fstore, dstore, astore, ret
                u2 index = (code.code[i + 2] << 8) | code.code[i + 3];
                std::cout << "(opcode " << static_cast<int>(opcode_wide) << ") #" << index;
        ...
        case 197: { // multianewarray
            ...
            std::cout << "multianewarray #" << index << " dim " << static_cast<int>(dimensions);
            i += 4;
            break;
        }
        case 198: { // ifnull
