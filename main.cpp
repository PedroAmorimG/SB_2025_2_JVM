...
int main(int argc, char *argv[]) {
  ...
  class_file.constant_pool =
      read_constant_pool(file, class_file.constant_pool_count, debug);

  class_file.access_flags = read_access_flags(file, debug);
  class_file.this_class =
      read_this_class(file, class_file.constant_pool, debug);
  class_file.super_class =
      read_super_class(file, class_file.constant_pool, debug);

  class_file.interfaces_count = read_interface_count(file, debug);
  class_file.interfaces =
      read_interfaces(file, class_file.interfaces_count, debug);

  class_file.fields_count = read_field_count(file, debug);
  class_file.fields = read_fields(file, class_file.fields_count,
                                  class_file.constant_pool, debug);

  class_file.methods_count = read_methods_count(file, debug);
  class_file.methods = read_methods(file, class_file.methods_count,
                                    class_file.constant_pool, debug);

  class_file.attributes_count = read_attribute_count(file, debug);
  class_file.attributes = read_attributes(file, class_file.attributes_count,
                                          class_file.constant_pool, debug);
  ...
  if (debug) {
    ...
    if (class_file.fields_count > 0) {
      std::cout << "\n--- Fields (" << class_file.fields_count << ") ---"
                << std::endl;
    ...
    if (class_file.attributes_count > 0) {
      std::cout << "\n--- Class Attributes (" << class_file.attributes_count
                << ") ---" << std::endl;
